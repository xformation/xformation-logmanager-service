...
public class PipelineInterpreter implements MessageProcessor {
    ...
    private void appendProcessingError(Rule rule, Message message, String errorString) {
        ...
        if (message.hasField(Message.FIELD_XFLOG_PROCESSING_ERROR)) {
            message.addField(Message.FIELD_XFLOG_PROCESSING_ERROR, message.getFieldAs(String.class, Message.FIELD_XFLOG_PROCESSING_ERROR) + "," + msg);
        } else {
            message.addField(Message.FIELD_XFLOG_PROCESSING_ERROR, msg);
        }
    }
