...
public class IndexRangesResource extends RestResource {
    ...
    public IndexRangeSummary show(
            @ApiParam(name = "index", value = "The name of the logmanager-managed Elasticsearch index", required = true)
            @PathParam("index") @NotEmpty String index) throws NotFoundException {
        if (!indexSetRegistry.isManagedIndex(index)) {
            throw new BadRequestException(index + " is not a logmanager-managed Elasticsearch index.");
        }
