...
public class DecodingProcessor implements EventHandler<MessageEvent> {
    ...
    private Message postProcessMessage(RawMessage raw, Codec codec, String inputIdOnCurrentNode, String baseMetricName, Message message, long decodeTime) {
        ...
        for (final RawMessage.SourceNode node : raw.getSourceNodes()) {
            switch (node.type) {
                case SERVER:
                    ...
                    if (message.getField(Message.FIELD_XFLOG_SOURCE_INPUT) != null) {
                        LOG.debug("Multiple server nodes ({} {}) set for message id {}",
                                message.getField(Message.FIELD_XFLOG_SOURCE_INPUT), node.nodeId, message.getId());
                    }
                    message.addField(Message.FIELD_XFLOG_SOURCE_INPUT, node.inputId);
                    message.addField(Message.FIELD_XFLOG_SOURCE_NODE, node.nodeId);
                    break;
                case RADIO:
                    // Always use the last source node.
                    if (message.getField(Message.FIELD_XFLOG_SOURCE_RADIO_INPUT) != null) {
                        LOG.debug("Multiple radio nodes ({} {}) set for message id {}",
                                message.getField(Message.FIELD_XFLOG_SOURCE_RADIO_INPUT), node.nodeId, message.getId());
                    }
                    message.addField(Message.FIELD_XFLOG_SOURCE_RADIO_INPUT, node.inputId);
                    message.addField(Message.FIELD_XFLOG_SOURCE_RADIO, node.nodeId);
                    break;
            }
        }
        ...
        if (remoteAddress != null) {
            ...
            message.addField(Message.FIELD_XFLOG_REMOTE_IP, addrString);
            if (remoteAddress.getPort() > 0) {
                message.addField(Message.FIELD_XFLOG_REMOTE_PORT, remoteAddress.getPort());
            }
            if (remoteAddress.isReverseLookedUp()) { // avoid reverse lookup if the hostname is available
                message.addField(Message.FIELD_XFLOG_REMOTE_HOSTNAME, remoteAddress.getHostName());
            }
